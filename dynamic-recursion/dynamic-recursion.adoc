...
Some cases are just too complex to solve with recursive queries. Let's take the case of a food manufacturer with a slightly more complicated recipe data model. The EQL recursive query solution requires that the same key be present at each level of traversal, i.e. we have a single element that may contain itself. We'll see how this becomes an issue in certain scenarios.
...
===== **TODO:** Discuss possible solutions
