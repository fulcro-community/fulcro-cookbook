...
In addition, the connection between the child and parent holds the element's qty and other pertinent information (e.g., the unit of measure `uom`). In other words, we have two mutually recursive elements - a recipe and a line item.
...
However, some cases are just too complex to solve with recursive queries. The EQL recursive query solution requires that the same key be present at each level of traversal, i.e. we have a single element that may contain itself. That is not the case here, where we have Recipe containing LineItem, containing Recipe... .

===== Option A: Flatten the data to get a self-recursive element

We could change our data so that a recipe contains directly sub-recipes, for example by splitting the quantity and uom data and the sub-recipes:

[source,clojure]
----
{:recipe/id 1
 :recipe/name "Pancake Batter"
 :recipe/line-items [{:r.l.i/qty 4
                      :r.l.i/uom "piece"
                      :r.l.i/ingredient {:ingredient/name "Eggs", :ingredient/id 101}}
                     {:r.l.i/qty 0.5
                      :r.l.i/uom "litre"
...
Then inside the component you would join the two pieces of data together.

It might not be elegant, but it gets the job done.

===== Option B: Use a dynamic, hook-based component to load the sub-recipe

Our secondary requirement is lazy loading of sub-recipes. We can solve both problems by using a dynamic component, i.e. one that is not part of the query tree but gets its data independently, via the `use-component` hook.
...
Below we can see what the component instances look like, for a recipe with two line items, one of which is a sub-recipe: 
...
TODO: Illustrate how side-effect (load/mutation/progress/web socket push, etc.) populates some portion of that tree, via a remote or data merge with a query fragment.
...
// TODO: I do not think this insight is relevant anymore, after I removed the union component? [jakub]
